...
import sys
def binary_cross_entropy(pred_output, labels):  #BCE LOSS
    loss_fct = torch.nn.BCELoss()
    m = nn.Sigmoid()
    n = torch.squeeze(m(pred_output), 1) # 将预测输出通过 Sigmoid 函数并去除维度为1的维度
    loss = loss_fct(n, labels) # 3. 计算二分类交叉熵损失
    return n, loss
...
class BINDTI(nn.Module):
    def __init__(self, device='cuda', **config):
        ...
        self.protein_extractor = HorNetModule(protein_emb_dim, num_filters, protein_num_head, protein_padding)
...
class HorNetModule(nn.Module):
    def __init__(self, embedding_dim, num_filters, padding=True):
        super(HorNetModule, self).__init__()
        if padding:
        ...
        self.in_ch = in_ch[-1]  
        self.hor1 = HorNet(in_planes=in_ch[0], out_planes=in_ch[1]) 
        self.bn1 = nn.BatchNorm1d(in_ch[1])
        self.hor2 = HorNet(in_planes=in_ch[1], out_planes=in_ch[2])
        self.bn2 = nn.BatchNorm1d(in_ch[2])
        self.hor3 = HorNet(in_planes=in_ch[2], out_planes=in_ch[3])
    ...
    def forward(self, v):
        ...
        v = v.transpose(2, 1)
        v = self.bn1(F.relu(self.hor1(v.unsqueeze(-2))).squeeze(-2))
        v = self.bn2(F.relu(self.hor2(v.unsqueeze(-2))).squeeze(-2))
        v = self.bn3(F.relu(self.hor3(v.unsqueeze(-2))).squeeze(-2))
