...
class Trainer(object):
    ...
    def train_epoch(self):
        ...
        for i, (v_d, v_p, labels) in enumerate(tqdm(self.train_dataloader)):
            self.step += 1    # step
            v_d, v_p, labels = v_d.to(self.device), v_p.to(self.device), labels.float().to(self.device) #to_device
            self.optim.zero_grad()  #zero_grad

            v_d, v_p, f, score = self.model(v_d, v_p) 
            if self.n_class == 1:
                n, loss = binary_cross_entropy(score, labels) # BCE Loss    
            else:
